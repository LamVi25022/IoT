...
const { mqttClient } = require("./mqttConnect");
const { mysqlConnection } = require("./mqttConnect");
 ...
 */
app.post("/devide_action/led/:action", (req, res) => {
 ...
 */
app.post("/devide_action/fan/:action", (req, res) => {
  const action = req.params.action;
...
app.get("/dash_board", (req, res) => {
  ...
  mysqlConnection.query(
    "SELECT * FROM data_sensor ORDER BY created_at DESC LIMIT 1",
...
app.get("/data_sensor/:page", (req, res) => {
  ...
  mysqlConnection.query(
    ...
    (error, results, fields) => {
      ...
      results.forEach((result) => {
        result.created_at = moment(result.reated_at)
          .tz("Asia/Ho_Chi_Minh")
          .format();
      });
      // Trả về kết quả dưới dạng JSON
...
app.get("/order/:subject/:order/:page", (req, res) => {
  ...
  const query = `SELECT * FROM data_sensor ${orderByClause} LIMIT ${pageSize} OFFSET ${offset}`;
...
app.get("/search/:type/:value/:page", (req, res) => {
  ...
  if (type === "hour") {
    query =
      "SELECT * FROM data_sensor WHERE HOUR(created_at) = ?  LIMIT ? OFFSET ?";
    queryParams = [value, pageSize, offset];
  } else if (type === "day") {
    query =
      "SELECT * FROM data_sensor WHERE DATE(created_at) = ?  LIMIT ? OFFSET ?";
    queryParams = [value, pageSize, offset];
  } else {
    query = "SELECT * FROM data_sensor WHERE ?? like ? LIMIT ? OFFSET ?";
...
app.get("/search/:type/:value/:type2/:order/:page", (req, res) => {
  ...
  if (type === "hour") {
    query = "SELECT * FROM data_sensor WHERE HOUR(created_at) = ?";
    queryParams = [value];
  } else if (type === "day") {
    query = "SELECT * FROM data_sensor WHERE DATE(created_at) = ?";
    queryParams = [value];
  } else {
    query = "SELECT * FROM data_sensor WHERE ?? like ?";
 ...
 *         description: Internal Server Error
 */
app.get("/devide_action/:page", (req, res) => {
  ...
  mysqlConnection.query(
    "SELECT * FROM devide_action LIMIT ? OFFSET ?",
    [pageSize, offset],
    (error, results, fields) => {
      if (error) {
        console.error("Error querying database:", error);
...
app.get("/devide_action/search/:field/:value/:page", (req, res) => {
...
app.get(
  "/devide_action/search/:field/:value/:page/:order/:type",
  (req, res) => {
